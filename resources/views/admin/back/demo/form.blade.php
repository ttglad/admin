            ...
            <div class="tab-content">
                ...
                <div class="tab-pane active" id="tab_1">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="text" autocomplete="off" value="{{ old('text') }}"
                               placeholder="文本框">
                    </div>
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="checkbox" name="flag[]"
                                   value="l" {{ (check_string($flag, 'l') === true) ? 'checked' : '' }}>
                            <label class="choice" for="flag[]" title="可用于友情链接" data-value="l" style="cursor: pointer;">[l]链接</label>
                            <input type="checkbox" name="flag[]"
                                   value="f" {{ (check_string($flag, 'f') === true) ? 'checked' : '' }}>
                            <label class="choice" for="flag[]" title="可用于页面幻灯片模型" data-value="f"
                                   style="cursor: pointer;">[f]幻灯</label>
                            <input type="checkbox" name="flag[]"
                                   value="s" {{ (check_string($flag, 's') === true) ? 'checked' : '' }}>
                            <label class="choice" for="flag[]" title="可用于页面滚动效果的文章" data-value="s"
                                   style="cursor: pointer;">[s]滚动</label>
                            <input type="checkbox" name="flag[]"
                                   value="h" {{ (check_string($flag, 'h') === true) ? 'checked' : '' }}>
                            <label class="choice" for="flag[]" title="可用于页面推荐性文章" data-value="h"
                                   style="cursor: pointer;">[h]热门</label>
                        </div>
                    </div>
                    <div class="form-group">
                        <label>下拉单选项 - 分类
                        ...
                        <div class="input-group">
                            <select data-placeholder="选择下拉项..." class="chosen-select" style="min-width:200px;"
                                    name="cat_id">
                    ...
                    <div class="form-group">
                        <label>图片上传 <a href="javascript:void(0);" class="uploadPic" data-id="thumb"><i
                                        class="fa fa-fw fa-picture-o" title="上传"></i></a> <a href="javascript:void(0);"
                                                                                             class="previewPic"
                                                                                             data-id="thumb"><i
                                        class="fa fa-fw fa-eye" title="预览小图"></i></a></label>
                        <input type="text" class="form-control" id="thumb" name="picture" value="{{ old('picture') }}"
                               placeholder="图片地址：如{{ url('') }}/assets/img/yas_logo.png" readonly="readonly">
                    </div>
                    <div class="form-group">
                        <label>文档上传 <a href="javascript:void(0);" class="uploadFile" data-id="file"><i
                                        class="fa fa-fw fa-file-o" title="上传"></i></a></label>
                        <input type="text" class="form-control" id="file" name="document" value="{{ old('document') }}"
                               placeholder="文件地址：如{{ url('') }}/assets/doc/yas_logo.pdf">
                    </div>
                    <div class="form-group">
                        <label>多行文本输入域
                            <small class="text-red">*</small>
                        </label>
                        <textarea class="form-control" name="textarea" rows="3" cols="200" autocomplete="off"
                                  placeholder="请输入文本">{{ old('textarea') }}</textarea>
                    </div>
                    <div class="form-group">
                        <label>富文本编辑器
                            <small class="text-red">*</small>
                ...
                <div class="tab-pane" id="tab_2">
                    <div class="form-group">
                        <label>日期选择输入框</label>
                        <input type="text" class="form-control" name="date_picker" minlength="10" 　maxlength="10"
                               placeholder="编辑时间" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',skin:'twoer'})"
                               value="{{ old('date_picker', date('Y-m-d H:i:s')) }}">
                    </div>
                </div><!-- /.tab-pane -->
