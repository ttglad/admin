...
@section('content')
    ...
    <div class="box box-primary">
        <div class="box-header with-border">
            ...
            @can('log-search')
                <div class="box-tools">
                    <form action="{{ route('admin_log') }}" method="get" class="form-inline">
                        ...
                        <div class="form-group">
                            <input type="text" class="form-control input-sm pull-right" name="s_operator_realname"
                                   value="{{ request('s_operator_realname') }}" style="width: 150px;"
                                   placeholder="搜索操作者真实姓名">
                        </div>

                        <div class="form-group">
                            <input type="text" class="form-control input-sm pull-right" name="s_operator_ip"
                                   value="{{ request('s_operator_ip') }}" style="width: 150px;" placeholder="搜索操作者IP">
                        </div>
            ...
            <table class="table table-hover table-bordered">
                ...
                @foreach ($system_logs as $sys_log)
                    <tr>
                        <td>
                            @can('log-show')
                                <a href="{{ route('admin_log_show', $sys_log->id) }}" class="layer_open"
                                   data-title="查看" data-width="400"><i class="fa fa-fw fa-link" title="查看"></i></a>
                            @endcan
                        </td>
                        <td class="text-red">{{ $log_type[$sys_log->type] }}</td>
                        <td class="text-green">{{ $sys_log->user->name or '--' }}
                            / {{ $sys_log->user->nickname or '--' }}</td>
